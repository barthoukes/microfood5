...
class CSortedItemList : Iterable<CSortedItem>
{
                     ...
                     withStatiegeld: Boolean): CMoney
    {
        ...
        for (item in m_sortedItems)
        {
            if ((item.getLocations() and locations) == 0 &&
                location != EItemLocation.ITEM_ALL_LOCATIONS
            )
            {
                continue
            }
            if (item.isValidItem(payStatus))
            {
                if (withStatiegeld)
                    total = total + item.getTotal()
                else total = total + item.getTotalWithoutStatiegeld()
            }
        }
        return total
    }
    ...
    fun getTotalFromIndex(line: Int): CMoney
    {
        return m_sortedItems[line].getTotal()
    }

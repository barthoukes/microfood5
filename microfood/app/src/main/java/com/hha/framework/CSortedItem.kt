...
class CSortedItem : Iterable<CItem> {
   ...
   fun merge(new_item : CSortedItem) : Boolean
    ...
    fun mergeSame(newItem: CSortedItem, compareStructure: Boolean, isKitchenPrintout: Boolean,
                  lowestTimeFrame: ETimeFrameIndex, highestTimeFrame: ETimeFrameIndex): Boolean {
        val mergeOnlyPortionChanges = false
        if (items[0].sequence == newItem.items[0].sequence) {
            return performMerge(newItem)
        }
        if (compareStructure && isSimilarItemStructureAndOrder(newItem,
    ...
    fun isSimilarItemStructureAndOrder(new_item: CSortedItem, is_kitchen_printout: Boolean,
                               ...
                               highestTimeFrame: ETimeFrameIndex): Boolean {
        if ( done != new_item.done)
        {
            return false;
        }
    ...
    fun getPrinterSpices(language: ETaal, perItem: Boolean): String {
        ...
        items.drop(1).forEach { item ->
            if (item.getQuantity() != 0 && (item.level == EOrderLevel.LEVEL_SPICES ||
                ...
                builder.append(if (language.isChinese())
    ...
    // Similar implementation for getPrinterExtras()
    // ...
